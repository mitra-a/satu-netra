...
import { UUID } from '../constanta/Index';
...
const VALUE = {
    'worry-hipnoterapi' : 'Khawatir : Mendengarkan Audio Terapi',
    'stress-hipnoterapi' : 'Stress : Mendengarkan Audio Terapi',
    'worry-music' : 'Khawatir : Mendengarkan Musik',
    'insomnia-music-nature' : 'Insomnia : Mendengarkan Musik dari Alam',
    'insomnia-music-lofi' : 'Insomnia : Mendengarkan Musik Tempo Lambat',
    'stress-music' : 'Stress : Mendengarkan Musik',
};
...
class SatuNetraAudio extends React.Component{
    ...
    async changeData(item){
        ...
        let date = Date.now();
        let value = {
            createdAt : new Date(date).toLocaleString(),
            history : VALUE[item],
            id : UUID
        }

        await History(value , 'ADD')
    }
    ...
    async isDone(){
        ...
        if(this.index < this.count){
            ...
            await this.setState({url : 'nothing'},function(){
                this.props.done();
            })
